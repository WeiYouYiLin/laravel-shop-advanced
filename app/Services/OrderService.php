...
class OrderService
{
    ...
    // 秒杀 将原本的 UserAddress 类型改成 array 类型
    public function seckill(User $user, /*UserAddress $address*/ array $addressData, ProductSku $sku)
    {
        // 将 $addressData 传入匿名函数
        $order = \DB::transaction(function () use ($user, /*$address*/ $addressData, $sku) {
            // 更新此地址的最后使用时间
            //$address->update(['last_used_at' => Carbon::now()]);
            // 创建一个订单
            $order = new Order([
